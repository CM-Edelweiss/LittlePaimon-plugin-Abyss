...
async def abyss_config_g():
    ...
    config["验证米游社签到开始时间"] = datetime.datetime(
        1970, 1, 1, hour=config["验证米游社签到开始时间(小时)"], minute=config["验证米游社签到开始时间(分钟)"]
    ).strftime("%H:%M")
    config["验证星铁签到开始时间"] = datetime.datetime(
        1970, 1, 1, hour=config["验证星铁签到开始时间(小时)"], minute=config["验证星铁签到开始时间(分钟)"]
    ).strftime("%H:%M")
    config["验证米游币开始执行时间"] = datetime.datetime(
        1970, 1, 1, hour=config["验证米游币开始执行时间(小时)"], minute=config["验证米游币开始执行时间(分钟)"]
    ).strftime("%H:%M")
...
async def abyss_config(data: dict):
    ...
    if "验证星铁签到开关" in data:
        temp_time = datetime.datetime.strptime(data["验证星铁签到开始时间"], "%H:%M")
        data["验证星铁签到开始时间(小时)"] = temp_time.hour
        data["验证星铁签到开始时间(分钟)"] = temp_time.minute
