...
from .api import query_score, tilioc
...
class MihoyoBBSCoin:
    ...
    def __init__(self, cookies, uid, user_id, re):
        self.headers: dict = {
        ...
        for i in self.mihoyo_bbs_List:
            ...
            if data["retcode"] != 0:
                ...
                self.state = (
                    "Cookie已失效"
                    if data["retcode"] in [-100, 10001]
                    else f"出错了:{data['retcode']} {data['message']}"
                    if data["retcode"] != 1034
                    else "遇验证码阻拦"
                )
    ...
    async def read_posts(self):
        ...
        for i in range(self.Task_do["bbs_Read_posts_num"]):
            req = await aiorequests.get(
                url=bbs_Detailurl.format(self.postsList[i][0]), headers=self.headers
            )
            data = req.json()
            if data["message"] == "OK":
                num_ok += 1
    ...
    async def share_post(self):
        ...
        for _ in range(3):
            req = await aiorequests.get(
                url=bbs_Shareurl.format(self.postsList[0][0]), headers=self.headers
            )
            data = req.json()
            if data["message"] == "OK":
                return "分享帖子：完成！"
...
async def mhy_bbs_coin(user_id: str, uid: str, re: bool) -> str:
    """
    执行米游币获取任务
    :param user_id: 用户id
    :param uid: 原神uid
...
@scheduler.scheduled_job(
    "cron", hour=config.myb_hour, minute=config.myb_minute, misfire_grace_time=10
)
async def _():
    await bbs_auto_coin()
...
async def bbs_auto_coin():
    ...
    logger.info(
        "米游币自动获取",
        f"开始执行加强米游币自动获取，共<m>{len(subs)}</m>个任务，预计花费<m>{round(100 * len(subs) / 60, 2)}</m>分钟",
    )
