...
from LittlePaimon.database import MihoyoBBSSub, LastQuery
...
async def sign_in(user_id, uid, re: bool) -> str:
    logger.info("米游社加强签到", "➤", {"用户": user_id, "UID": uid}, "开始执行签到！", True)
    ...
    if sign_info and "data" in sign_info and sign_info["data"]:
        ...
        logger.info(
            "米游社加强签到", "➤", {"用户": user_id, "UID": uid}, "出错, 请检查Cookies是否过期！", False
        )
        return False, "签到失败...请检查Cookies是否过期！"
    # 检测是否已签到
    if sign_info["is_sign"]:
        logger.info("米游社加强签到", "➤", {"用户": user_id, "UID": uid}, "今天已经签过了", True)
        global already
        already += 1
    ...
    for index in range(4):
        ...
        if (
            ...
            # 成功签到!
                ...
                if index == 0:
                    logger.info(
                        "米游社加强签到", "➤", {"用户": user_id, "UID": uid}, f"该用户无校验码!", True
                    )
                    result = "[无验证]"
                else:
                    logger.info(
        ...
        # 重试超过阈值
            ...
            logger.info(
                "米游社加强签到", "➤", {"用户": user_id, "UID": uid}, f"超过请求阈值...", False
            )
    ...
    # 签到失败
        ...
        result = "签到失败!"
        logger.info(
            "米游社加强签到", "➤", {"用户": user_id, "UID": uid}, f"签到失败, 结果: {result}", False
        )
        return False, result
    # 获取签到列表
...
@scheduler.scheduled_job(
    "cron", hour=config.enable_hour, minute=config.enable_minute, misfire_grace_time=10
)
async def _():
    await bbs_auto_sign()
...
async def bbs_auto_sign():
    ...
    logger.info(
        "米游社加强签到",
        f"开始执行米游社加强签到，共<m>{len(subs)}</m>个任务，预计花费<m>{round(100 * len(subs) / 60, 2)}</m>分钟",
    )
    coin_result_group = defaultdict(list)
    for sub in subs:
        im, result = await sign_in(str(sub.user_id), sub.uid, True)
        if (not im) and ("Cookie" in result):
            ...
            if sub := await MihoyoBBSSub.get_or_none(**sub_data):
                logger.info(
                    "米游社加强签到",
                    "➤",
                    {"用户": str(sub.user_id), "UID": sub.uid},
                    "ck失效已经自动取消签到",
                    False,
                )
                await sub.delete()
    ...
    for group_id, result_list in coin_result_group.items():
        ...
        try:
            ...
            logger.info("米游社加强签到", "➤➤", {"群": group_id}, f"发送结果失败: {e}", False)
        await asyncio.sleep(random.randint(3, 6))

    logger.info("米游社加强签到", f"获取完成，共花费<m>{round((time.time() - t) / 60, 2)}</m>分钟")
